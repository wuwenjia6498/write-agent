...
本项目是一套 **工业级 AI 写作协作系统**，旨在将"老约翰儿童阅读"品牌 15 年的内容调性通过结构化的 **两层判断机制** + **9 步 SOP 工作流** 进行工业化产出。系统以 **Claude（Anthropic）** 为核心创作模型，**OpenAI Embedding** 为语义检索引擎，深度集成品牌资产管理、频道风格建模、知识库 RAG 检索、五条最高军规、**v5.0 相关性匹配样文抽取 + XML 隔离**、四遍审校降 AI 味、**划词 AI 局部重写**等能力，打造从选题到终稿的全流程闭环。
...
这是系统最核心的引擎，在 `workflow_engine.py` 中实现（1382 行）：
...
│  │     含大纲、工作量评估、优劣分析                     │
│  ↓                                                    │
│  S4 ── 协作文档 (v4.5 知识库感知) ★ 卡点              │
│  │     AI 评估已有参考资料充足度，主动承担融入任务       │
│  │     仅当核心论据极度缺乏时以【可选补充】提问          │
│  │     用户可补充自由文本信息 → 注入 Step 7             │
│  ↓                                                    │
│  S5 ── 风格建模（v4.5 极简模式，自动流转）             │
│  │     统计频道可用样文数量，不再进行 6 维特征分析       │
│  │     Step 7 将基于选题相关性智能抽取 Top 2 样文       │
│  ↓                                                    │
│  S6 ── 创作准备（自动流转，无卡点）                    │
│  │     自动封装 RAG 检索事实 + 标杆样文特征上下文       │
│  ↓                                                    │
│  S7 ── 初稿创作 (v5.0 相关性匹配 + XML 隔离) ★★       │
│  │     废弃 random.sample，基于选题关键词相关性评分      │
│  │     掐头取尾黄金比例截取（前700字+后300字）           │
│  │     XML 边界隔离格式包裹样文 + 严厉防抄袭警告        │
│  │     五条最高军规 + 全局禁书单深度注入                 │
...
| **L4** | 五条最高军规 + XML 隔离 (Step 7 v5.0) | `execute_step_7()` Prompt | 源头阻断 AI 式行文 + 防样文事实泄漏 |
| **L5** | 四遍纪律审校 + 频道铁律 | `execute_step_8()` + `_build_channel_rules_prompt()` | 逻辑→准确性→语气→排版 全面修复 |

### 7.2 五条最高军规（Step 7 核心约束 v5.0）
...
| 注入点 | 作用 |
|:---|:---|
| **Step 3 选题讨论** | 选题方案生成时注入禁书清单，避免选题中引用禁书 |
| **Step 4 协作文档** | 「最高红线」强制使用知识库书目替代禁书 |
| **Step 7 初稿创作** | 五条军规第 3 条明令禁止使用禁书 |
| **Step 8 四遍审校** | 第二遍「禁用书目清理法则」执行泛化抹除 |
| **划词重写 API** | `ai_rewrite.py` 注入防禁书机制，即便用户要求换书也绝不使用禁书 |

当前禁书单：从 `writing_constraints.json` 动态加载（如 `《夏洛的网》《小王子》《窗边的小豆豆》` 等）
...
| **v4.5** | **Step 4 知识库感知协作文档**：Step 4 升级为卡点，AI 先评估已有参考资料充足度并主动承担融入任务，仅当核心论据极度缺乏时向用户提问（最多 2-3 个简短问题）；用户可回复"无"或直接跳过。**Step 5 极简降维**：废弃 6 维特征分析与智能匹配推荐，改为仅统计频道可用样文数量，样文选取逻辑下沉至 Step 7。 |
| **v5.0** ★ | **Step 7 三项核心改造**：① 废弃 `random.sample` 盲抽，基于选题关键词的中文文本相关性评分抽取 Top 2 样文；② 掐头取尾黄金比例截取（前 700 字 + 后 300 字），保留开篇节奏与结尾升华；③ XML 边界隔离格式（`<style_reference_samples>`）包裹样文，附严厉防抄袭警告，杜绝模型照搬样文事实。**划词重写温度调优**：temperature 从 0.6 提升至 1.0，增强重写多样性。**频道专属内容铁律**：新增 `_build_channel_rules_prompt()` 动态注入频道级 must_do / must_not_do 规则至 Step 7 和 Step 8。 |
...
### 15.3 Step 7 v5.0 相关性匹配 + 掐头取尾 + XML 隔离

- **废弃随机抽取**：不再使用 `random.sample` 盲抽，改为基于选题关键词的轻量级中文文本相关性评分，抽取 Top 2 样文
- **掐头取尾黄金比例截取**：超过 1000 字的样文只保留前 700 字（开篇节奏）+ 后 300 字（结尾升华），中间部分省略
- **XML 边界隔离**：样文以 `<style_reference_samples>` XML 标签包裹，附严厉警告"绝对禁止照抄 `<content>` 中的任何具体事实、人名或案例"，防止模型混淆样文内容与创作要求
- 通过样文原文让大模型直接感知行文风格，实现**结构像素级镜像**

### 15.4 Step 4 v4.5 知识库感知协作文档

- Step 4 从自动流转升级为卡点，等待用户补充信息
- AI 先评估已有【补充参考资料】（知识库 RAG）和【调研摘要】的充足度
- 已覆盖的内容 AI 主动承担，仅当极度缺乏时以【可选补充】提问（最多 2-3 个简短问题）
- **原因**：最大限度降低用户输入负担，AI 能自行解决的绝不麻烦用户

### 15.5 Step 5 极简降维（v4.5）

- 废弃 6 维特征分析、智能匹配推荐、风格画像合成等复杂逻辑
- Step 5 仅统计频道可用样文数量，提示 Step 7 将自动抽取
- 样文选取和截取的核心逻辑全部下沉至 Step 7 执行
- **原因**：降维后 Prompt 更简洁、Step 5 不再成为阻断卡点、模型遵从性更高

### 15.6 Step 7 五条军规 + Step 8 四遍审校（v3.7 → v5.0）

- Step 7 在创作时即注入五条最高军规，从源头阻断 AI 式行文
- Step 7 新增频道专属内容铁律（`_build_channel_rules_prompt()`）动态注入 must_do / must_not_do
- Step 8 接管所有硬性合规检查（屏蔽词、禁书泛化抹除、字数控制）
- **原因**：源头截杀 + 末端兜底双保险，最大程度降低 AI 检测率

### 15.7 划词重写精确索引替换 vs string.replace
...
| SQL Schema | ~240 行 |
| JSON 配置 | ~300+ 行 |
| 核心引擎 (`workflow_engine.py`) | 1,382 行 |
| 核心页面 (`workbench/page.tsx`) | 2,091 行 |
| 数据库服务 (`db_service.py`) | 1,390 行 |
| 知识库管理页 (`admin/knowledge/page.tsx`) | 1,136 行 |
| 工作流路由 (`workflow.py`) | 1,018 行 |
| 划词重写组件 (`ArticleEditor.tsx`) | 434 行 |
| ORM 模型定义 (`models.py`) | 737 行 |
| API 路由总数 | 45+ 个端点 |
| 数据库表 | 7 张 |
| 数据库索引 | 12+ 个 |
| Python 依赖 | 30+ 个包 |
...
2. **品牌化**：三频道 × 独立人格 × 相关性匹配样文 × XML 隔离 × 结构像素级镜像 = 工业化品牌调性
3. **去 AI 化**：五层防护体系（五条军规源头截杀 + 屏蔽词 + RAG 真实素材 + 禁书深度注入 + 四遍审校 + 频道铁律动态注入）系统性降低 AI 检测率
