# -*- coding: utf-8 -*-
"""
脚本 C: 导入绘本阅读文档到 RAG 知识库 (docx → KnowledgeChunk)

数据源目录:
  - data_source/picture_books/book_lists/  → channel_scope='picture_books', material_type='booklist'
  - data_source/picture_books/qa_guides/   → channel_scope='picture_books', material_type='qa'
...
DIRECTORY_CONFIG = [
    {
        "path": os.path.join(PROJECT_ROOT, "data_source", "picture_books", "book_lists"),
        "channel_scope": "picture_books",
        "material_type": "booklist",
    },
    {
        "path": os.path.join(PROJECT_ROOT, "data_source", "picture_books", "qa_guides"),
        "channel_scope": "picture_books",
        "material_type": "qa",
    },
]
...
def process_directory(config: dict, embeddings_model: OpenAIEmbeddings) -> int:
    ...
    if not files:
        print(f"[WARN] 目录为空（无 .docx 文件）: {dir_path}")
        return 0
...
def main():
    ...
    print("\n" + "=" * 60)
    print(f"  [DONE] 总计导入 {grand_total} 个知识切片 (picture_books)")
    print("=" * 60)
