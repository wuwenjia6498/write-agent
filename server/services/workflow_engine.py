...
**⛔ 铁律（逐条自检，缺一不可）**：
1. **≤ 25 字**（含标点）——超过即废稿，写完请数字数！
2. **引号 ≤ 1 对**（书名号不算）——0 对最佳，绝不允许 2 对及以上
3. **禁止 从"A"到"B" 句式**——"从X到Y""从X走向Y"全部禁止，出现即废稿
4. **禁止贩卖焦虑**

**4个选题请使用不同的标题风格，根据选题内容选择最匹配的风格**（不要重复使用同一种）：
- 口语化设问：如《孩子读了很多书，为什么写作还是不行？》（0对引号，19字）
- 具体场景切入：如《当孩子合上书说"我看完了"，阅读才刚开始》（0对引号，19字）
- 直给观点：如《低年级孩子最需要的阅读能力，课本里教不了》（0对引号，20字）
- 朴素悬念：如《那些写作好的孩子，读书时都多做了一步》（0对引号，18字）

请模仿以上示例：短、白话、不堆比喻、不加引号也能成立。
...
> 以下资料来自知识库，既包含老约翰的专业理论、阅读指导卡片，也可能包含真实的课堂案例/微素材。
...
【动笔前的最高军规（系统级硬性校验）】
=========================================
请在开始输出正文前，将以下五条军规刻在脑子里。系统将对输出进行严格词表扫描，触碰底线即废稿！

⛔ 1. **【禁止"不是…而是…"及所有变体】**：全文（含标题）不得出现"不是X，而是Y"、"不在X，而在Y"、"而不是"、"不仅仅是X，更是Y"、"不要X，而是Y"。想表达对比时，直接陈述正面观点即可。出现即废稿。

2. **【禁止编造人物、案例与违禁书目】**：严禁虚构"我朋友的孩子""收到一位妈妈留言"等个人叙事，必须用群体泛指（"很多孩子"、"不少家长"）。所有案例必须来自上方参考资料，绝不允许编造。禁用书目：{banned_books_list}，宁可少举例也绝不自编或使用禁用书目。

3. **【高级开场白】**：从以下三种策略中择一开场——A. 反常识引入（颠覆认知的观点）；B. 客观场景白描（群体性泛指，禁止给孩子起名加戏）；C. 直击核心概念。

4. **【结构镜像参考样文】**：精准识别并采用参考样文的文本骨架（扁平罗列 or 叙事推进），像素级模仿样文的段落节奏和层级深度。

5. **【自然行文基调】**：用人类作者自然平滑的叙述逻辑写作。禁止"什么是xxx？""怎么做呢？"等自问自答过渡，保持与读者平视共情的温润语气。引用具体案例或场景时，必须用时间/地点/背景状语自然引入（如"在读书会上""有一次课堂讨论中"），禁止无铺垫地突然跳入具体场景。
...
"""
        ...
        # 中文约 1.5~2 tokens/字，乘以 4 留足余量，支持 5000 字以上长文
        estimated_tokens = min(int(word_count * 4), 16000)
    ...
    async def execute_step_8(
        ...
        """
        Step 8: 纪律审校机制（v4.0 - 第零遍 + 四遍审校，文章与报告分离输出）
        
        职责：
        0. 第零遍：「不是…而是…」句式清零（最高优先级）
        1. 第一遍：逻辑把控 - 结构与行文逻辑
        2. 第二遍：知识准确性核对 - 频道底线 + 禁用书目
        3. 第三遍：语气润色 - 去 AI 腔 + 屏蔽词 + 自问自答 + 语气 + 伪学术
        4. 第四遍：排版与字数审校
        
        输出格式：修改后文章在前（===FINAL_ARTICLE===标记），审校报告在后（===AUDIT_REPORT===标记）
...
请对文章进行审校，发现问题直接**局部重写**修复，无需返回上一步。
...
逐句扫描全文（含标题），找出所有"不是…而是…"及其变体（不在…而在、而不是、不要…而是、不仅仅是…更是）。

**执行方式**：从第一段开始，逐段逐句检查。每发现一处，立即记录原句并改写。扫描完毕后，在报告中**逐条列出**：
- 原句（标注所在段落位置）
- 改写后的句子
- 如果全文确实无此句式，写"逐段扫描完毕，全文 X 段共 Y 句，确认 0 处"（必须写出段数和句数，证明你真的扫描了）。

**所有匹配项必须改写为正面直述，一处不留。**改写策略参见第三遍屏蔽词表。
...
- 段落之间的过渡是否自然连贯？是否有突然跳入具体场景却缺少时间/地点/背景状语的情况？如有，补充引导性状语（如"在一次读书会上""有老师分享过这样一个经历"）。
...
| 禁用短语 | 替换为 | 原因 |
|---------|-------|------|
{chr(10).join(blocked_phrases_with_replacement)}

**替换原则**：优先使用上表建议；不适合语境时自行调整，保持口语化、有温度。
**超越词表**：除上表外，也请主动识别书面语过重（如"意义重大"→"很重要"）、成语堆砌（如"各司其职"→"各有分工"）、绕弯子的表达，一并简化为白话。

### 自问自答清理
检查是否存在"什么是xxx？""怎么做呢？""为什么呢？"等生硬的自问自答过渡，替换为自然平滑的叙述衔接。

### 语气审查
确保全文保持与读者（家长）平视共情的温润基调。发现居高临下、指责或爹味说教语气，改写为温和引导式表达。

### 伪学术引用清理
检查"教育研究者发现""心理学研究表明"等无具体来源的伪学术引导语，直接删除引导语，保留结论即可。
...
- 拆分超过 40 字的长句
- 每段不超过 200 字，过长段落适当留白分段
- 标点符号使用是否自然
- 通过删减冗余使文章体量贴近目标要求
- **绝对禁止**在文章标题或末尾标注"全文共xxx字""修改后版本（xxx字）"等字眼，只输出纯净正文
...
===FINAL_ARTICLE_START===
（输出完整的修改后文章，确保所有审校问题已修复。此处只输出纯净正文，不含任何审校说明。）
===FINAL_ARTICLE_END===
...
"""
        ...
        think_aloud += "📋 审校流程（第零遍 + 四遍）：\n"
        think_aloud += "  ⛔ 第零遍：「不是…而是…」句式清零（最高优先级）\n"
        think_aloud += "  🔴 第一遍：逻辑把控（结构调整、起承转合优化）\n"
        think_aloud += "  🟡 第二遍：知识准确性核对（禁用书目 + 事实核查）\n"
        think_aloud += "  🟢 第三遍：语气润色（去 AI 腔 + 屏蔽词 + 自问自答 + 语气 + 伪学术）\n"
        think_aloud += "  🔵 第四遍：排版与字数审校\n\n"
        think_aloud += "💡 反馈机制：发现违禁内容将执行局部重写，无需返回 Step 7"
        
        # 审校输出 = 审校报告 + 完整修改后全文，token 消耗约为初稿的 2 倍
        estimated_tokens = min(int(word_count * 6), 16000)
        
        result = await ai_service.generate_content(
            system_prompt=system_prompt,
            user_message=f"请对以下文章执行第零遍+四遍审校（字数允许范围：{min_allowed}~{max_allowed}字）：\n\n{draft}",
            temperature=0.3,
            max_tokens=estimated_tokens
        )
